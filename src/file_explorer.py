...
def file_info(path: str) -> dict[str, str | int]:
    ...
    return {
        "name": p.name,
        "size": p.stat().st_size,
        "modified": datetime.fromtimestamp(p.stat().st_mtime, tz=UTC).strftime("%Y-%m-%d %H:%M:%S"),
    }


def days_until(target: str) -> int:
    target_date = datetime.strptime(target, "%Y-%m-%d").replace(tzinfo=UTC)
    return (target_date - datetime.now(UTC)).days
